...
class OfferServices
{
    ...
    function OfferOrderList(Store $store,$type = 1){
        ...
        if($type == 1){ //新订单
            ...
            return StoreOfferOrder::select($field)->where('offer_status',0)->whereIn('order_no',$orderNoList)->orderBy('created_at','desc')->paginate($limit);            
        }
        ...
        if($type == 2){
             $list = StoreOfferOrder::select($field)->leftJoin('store_offer_record','store_offer_record.order_id','=','store_offer_orders.id')
                ...
                ->whereIn('store_offer_orders.offer_status',[0,1])
