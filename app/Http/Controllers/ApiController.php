...
class ApiController extends Controller
{
    ...
    public function getFilmWithCinema(Request $request){
        ...
        if($schedulesList->isEmpty()){
            ...
            // $apiResult = (array) ApiHouse::getFilmShowByDate($cinema_id,date('Y-m-d 03:00:00'));
        ...
        foreach($list as $item){
            ...
            $item->datelist = $this->getShowDateList($openTime,$film_id);
        }
        return $this->success('',compact('cinema','list'));
    }
    
    //获取影片排片日期
    protected function getShowDateList($openTime,$film_id = 0){
        //聚福宝
        $date = array();
        if($film_id)
        {
    ...
    public function getCinemaWithFilm(Request $request){                  
        ...
        $list = Api\Cinema::search($this->cityId,$distId,$brand_id,$subway,$trade_area)->get();
   ...
   protected function getDistance($lat1, $lng1, $lat2, $lng2){
    ...
    public function currentFilmList(Request $req){
        ...
        if($type == 1){
            ...
            if($list->isEmpty()){
                ...
                Api\Film::syncData($apiResult,1,$cityId);
            }
            foreach($list as &$item){
                if(is_array($item)){
                    $item = (object)$item;
    ...
    public function cinemaBrand(){
        ...
        $common = array(array('id'=>-1,'brand_name'=>'全部'));
